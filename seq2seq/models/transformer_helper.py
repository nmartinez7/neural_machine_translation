...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        '''
        #print("state before: {}".format(state.shape))
        state, _ = self.self_attn(query=state, key=state, value=state, key_padding_mask=encoder_padding_mask)
        #print("state after: {}".format(state.shape))
        #print("encoder_padding_mask: {}".format(encoder_padding_mask.shape))
        ...
        ___QUESTION-6-DESCRIBE-D-START___
        1.  Add tensor shape annotation to EVERY TENSOR below (NOT just the output tensor)
        state (before self-attention): [src_lengths, batch_size, embed_dim]
        query : [src_lengths, batch_size, embed_dim]
        key: [src_lengths, batch_size, embed_dim]
        value: [src_lengths, batch_size, embed_dim]
        
        2.  What is the purpose of encoder_padding_mask? 
        The encoder_padding_mask is used to fill sequence spaces that are shorter than the max.
        sequence length in a batch. I.e. the shorter sentences ends' are padded with zeros.
        This length is always specific to the batch. 
        
        3.  What will the output shape of `state' Tensor be after multi-head attention?
        The output shape of state after multi-head attention is the same
        state: [src_lengths, batch_size, embed_dim]
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        #print("encoder_out : {}".format(encoder_out.shape))
    
        
        '''
        1.  Add tensor shape annotation to EVERY TENSOR below (NOT just the output tensor)
        state: [tgt_length, batch_size, embed_dim]
        attn_shape: [num_heads*batch_size, tgt_length, embed_dim/num_heads]
        key (encoder_out): [src_length, batch_size, embed_dim]       
        
        2.  How does encoder attention differ from self attention? 
        While self attention models how the input tokens interact with each other, i.e.
        how a sequence interacts with itself, the encoder attention (output and input, but encode???)
        
        3.  What is the difference between key_padding_mask and attn_mask? 
        The key_padding_mask is used to fill sequence spaces that are shorter than the max.
        sequence length in a batch. I.e. the shorter sentences ends' are padded with zeros.
        This length is always specific to the batch. The attn_mask, as described before,
        is used to prevent the decoder from attending "future" tokens at inference time. In other words,
        to prevent 
        
        4.  If you understand this difference, then why don't we need to give attn_mask here?
        We don't need attn_mask in the encoder, only in the decoder. Here we are calculating encoder
        attention.
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        k = self.head_embed_size
        
        """
        print("embed_dim: {}".format(embed_dim))
        print("k_embed_size: {}".format(self.k_embed_size))
        print("v_embed_size: {}".format(self.k_embed_size))
        print("tgt_time_steps: {}".format(tgt_time_steps))
        print("batch_size: {}".format(batch_size))
        print("num heads: {}".format(num_heads))
        """
        ####Linear projction of query key and value:
        #queries = self.q_proj(query).view(batch_size, tgt_time_steps, num_heads, k)
        #keys = self.k_proj(query).view(batch_size, tgt_time_steps, num_heads, k)
        #values = self.v_proj(query).view(batch_size, tgt_time_steps, num_heads, k)
        ...
        #print("queries shape: {}".format(queries.size()))
        #print("keys shape: {}".format(keys.size()))
        #print("values shape: {}".format(values.size()))
        
        queries = queries.view(batch_size, -1, num_heads, k) # [b, t, h, k]
        keys = keys.view(batch_size, -1, num_heads, k) # [b, t, h, k]
        values = values.view(batch_size, -1, num_heads, k) # [b, t, h, k]
        ...
        keys = keys.transpose(1,2).contiguous().view(batch_size*num_heads, -1, k)
        queries = queries.transpose(1,2).contiguous().view(batch_size*num_heads, -1, k)
        values = values.transpose(1,2).contiguous().view(batch_size*num_heads, -1, k)
        ...
        # Now do the dot product of query and key vector
        dot_q_k = torch.bmm(queries, keys.transpose(1,2))/self.head_scaling
        
        #dot_q_k = torch.matmul(queries.transpose(2,3), keys)/self.head_scaling
        
        #print("score size: {}".format(dot_q_k.size()))
        
        #############################
        if attn_mask is not None:
            
            dot_q_k += attn_mask.unsqueeze(0)
        
        #########################
        if key_padding_mask is not None:
            
            # Don't attend to padding symbols
            #print("dot_q_k size: {}".format(dot_q_k.size()))
            dot_q_k = dot_q_k.view(batch_size, num_heads, tgt_time_steps, keys.size(1))
            dot_q_k = dot_q_k.masked_fill(key_padding_mask.unsqueeze(1).unsqueeze(2), float('-inf'))
            dot_q_k = dot_q_k.view(batch_size*num_heads, tgt_time_steps, keys.size(1))
        #############################
        
        #Apply the softmax to tensor above
        dot_q_k = torch.nn.functional.softmax(dot_q_k, dim=-1)
        
        #Apply self-attention to values:
        attn_weights = torch.bmm(dot_q_k, values)#.view(num_heads, batch_size, -1, k)
        
        #Unify heads, do final projection:
        attn  = attn_weights.transpose(0,1).contiguous().view(-1, batch_size, embed_dim)
        #attn  = attn_weights.transpose(0,1).contiguous().view(-1, batch_size, embed_dim)
