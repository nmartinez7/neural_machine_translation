...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            """
            1.
            output: [batch_size, tgt_tokens, vocab_size], (vocab_size being the number of tokens (i.e. classes) in the target language)
            ...
            # Reshape the output for it to be in the shape batch_size*tgt_input X vocab_size, and the tgt_tokens
            # are also passed to the calculation of the cross entropy loss. The loss is normalized by the length
            ...
            # Compute the gradient of the loss tensor with respect to the model graph.
            loss.backward()
            ...
            #print("source lengths data: {}".format(sample['src_lengths'].shape))
            #print("source tokens: {}".format(sample['src_tokens']))
            #print("source tokens SHAPE: {}".format(sample['src_tokens'].shape))
            #print("tgt inputs: {}".format(sample["tgt_inputs"].shape))
            #print("tgt tokens: {}".format(sample["tgt_tokens"].shape))
            #print(output.view(-1, output.size(-1)).shape)
            #print( sample['tgt_tokens'].view(-1).shape)
